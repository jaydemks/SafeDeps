...
The strongest claim is scoped to tested local dependency policy gates. SafeDeps has a limited first npm runtime guard slice and strong NuGet scan/CI validation, but does not claim broad stable npm runtime protection, NuGet runtime protection, or public registry publishing until those areas have their own green, blocking evidence.
...
| npm and NuGet runtime protection | Limited/not claimed | npm has a first blocking runtime slice; NuGet runtime blocking remains outside stable claims because no `dotnet` command interceptor is installed. |

## Experimental Areas

npm and NuGet scanning are supported. npm runtime protection is limited to the first validated blocking slice; broader npm runtime protection remains experimental. NuGet runtime protection is not claimed until SafeDeps has a dedicated `dotnet` command interception design and green e2e evidence for it.
